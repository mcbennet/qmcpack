...
def remove_attribute(node, attrib_name):
...
def three_to_one(tree):
    wf_nodes = tree.findall('.//wavefunction')
    if len(wf_nodes) != 1:
        print('No wavefunction nodes found, or more than one')
        return
    wf_node = wf_nodes[0]
    sposet_nodes = wf_node.findall("./sposet")
    if len(sposet_nodes) > 0:
        print('Input is not consistent with style 3')
        return
    detset_nodes = wf_node.findall("./determinantset")
    detset_node = detset_nodes[0]
    sposet_nodes = detset_node.findall("./sposet")
    if len(sposet_nodes) > 0:
        print('Input is not consistent with style 3')
        return
    el = ET.Element('sposet')
    el.tail = '\n'
    wf_node.append(el)
    print(wf_node.head)
